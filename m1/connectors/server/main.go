...
func onAfterStart(_ context.Context, runtime *statefun.Runtime) error {
	...
	// Command source types.	mustTypeInit(dbc, types.TYPE_FOLIAGE_CONNECTOR_LSHW)
...
func start() {
	...
	if runtime, err := statefun.NewRuntime(*statefun.NewRuntimeConfigSimple(natsURL, apps.APP_CN_SERVER).UseJSDomainAsHubDomainName().SetDomainRoutersHandling(false)); err == nil {
		registerFunctionTypes(runtime)
		runtime.RegisterOnAfterStartFunction(onAfterStart, false)
		if err := runtime.Start(context.TODO(), cache.NewCacheConfig(apps.APP_CN_SERVER+"cache")); err != nil {
