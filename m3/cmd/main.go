...
func registerFunctionTypes(runtime *statefun.Runtime) {
	...
	// triggerfunc.RegisterObjectNameGenerator(runtime)
...
func onAfterStart(ctx context.Context, runtime *statefun.Runtime) error {
	...
	go healthyState(ctx)
	go kickerController(ctx, runtime)
...
func start() {
	...
	if runtime, err := statefun.NewRuntime(*statefun.NewRuntimeConfigSimple(natsURL, apps.APP_CMD).SetDomainRoutersHandling(false).UseJSDomainAsHubDomainName()); err == nil {
